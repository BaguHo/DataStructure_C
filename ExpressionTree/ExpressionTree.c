...
BTreeNode * MakeExpTree(char exp[]){
    ...
    // 1. 하나의 이진트리는 만들겠지만 그 이진트리를 자식노드로 하는 부모격의 이진트리를 만들지 못함
    // 2. 하나의 이진트리가 만들어졌을 때 그 pnode를 다음 StackPop에서 부모격의 이진트리로 어떻게 연결시켜야 하는지 
    // -> pnode가 for문이 한 번 끝나면 다시 MakeBTree()를 하는데 StackPop을 사용해 이진트리를 만들어도 
    // 다시 빈(NULL) 이진트리가 만들어지는 거 아닌가?
	...
	for(i=0; i<expLen; i++){
		...
		if(isdigit(exp[i])){ // 숫자면 push
            ...
            // Q: left_node, right_node를 굳이 따로 써야하나? 하나의 node로 해결할 수 있는 방법은 없을까?
            // 먼저 pop한 값이 오른쪽으로 가야한다.
            BTreeNode * right_node = MakeBTree();
            SetData(right_node, StackPop(&stack));
			MakeRightSubNode(pnode, right_node);
            
            if(IsStackEmpty(&stack)){ // temp를 pnode의 부모노드로 만들어서 temp를 return 시킨다
